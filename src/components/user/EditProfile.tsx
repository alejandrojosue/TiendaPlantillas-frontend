...
export default function EditProfile({ biography, githubLink, instagramLink, LinkedInLink, stripeLink, isFreelancer }: Props) {
          ...
          <div>
            ...
            {isFreelancer
              ? <label
                class="flex items-center gap-x-2 my-2 w-full text-slate-400 font-semibold"
              >
                <IconStripe width="35" height="35" />
                <input
                  type="text"
                  name="stripeLink"
                  value={stripeLink}
                  placeholder="Mi ID de mi cuenta de stripe"
                  class="w-full rounded-md my-1 bg-transparent border border-slate-400 p-1 text-slate-500 focus:text-slate-300"
                />
              </label>
              : ''}
